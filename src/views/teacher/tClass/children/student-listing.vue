<template>
  <div class="tClass">   
    <template v-if="!$route.meta.isChildren">
      ...
      <!-- 未加入班级学生列表 -->
      <el-table
        :data="$store.state.tClass.unJoinClassList"
        border
        style="width: 100%"
      >
...
export default {
  ...
  methods: {
    ...
    modifyStuInfoFn(obj) {
      ...
      this.$nextTick(() => {
        this.addStuForm.name = obj.name
        this.addStuForm.college = obj.college
        this.addStuForm.major = obj.major
        this.addStuForm.phone_number = obj.phone_number
      })
