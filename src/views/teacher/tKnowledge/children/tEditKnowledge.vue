<template>
  <div class="edit">
    <el-form ref="form" :model="pointDetail" :rules="rules" label-width="100px" v-if="pointDetail">
      ...
      <el-form-item label="难度" prop="level">
        <el-select placeholder="难度" v-model="pointDetail.level" :value="questionLevel(pointDetail.level)">
  ...
  methods: {
    onSubmit() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          const { knp_id, parent_knp_id, level, name, context, create_user } = this.pointDetail
          updatePoint(knp_id, parent_knp_id, level, name, context, create_user).then(res => {
            this.$message({
              type: 'success',
              message: '保存成功!'
            });
